# Platform Passkey
This document will guide a user through the requirements to use this python module as a system (platform) passkey authenticator; which emulates an HID device; using the UHID kernel module.
...
## Attestation and Credential Trust
Not surprisingly, there is no source of trust for any of the attestations generated by this module/app. This app (for the moment) also does not claim an AAGIUD.

By default, a user will be able to select between two credential "types": A UserVerified credential, where a pin must be provided as part of registration/authentication; and a UserPresent credential, which only requires an accept gesture (click a button, scan a fingerprint).
...
The `tmp2_pytss` package provides a python API wrapper to the system TPM 2.0 library. This stack is required if you want to store the platform key in the TPM enclave attached to your device.
...
- **Error access TPM with TSS**: Verify kernel header files exist, python binding are correct.
- **Permission denied on /dev/uhid**: Run `groups` to verify you're in the `uhid` group, then log out and back in
- **Browser/Client freezes during registrations or authentication**: Use the advanced configuration menu to restart the software authenticator device.
