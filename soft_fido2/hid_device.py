...
class Authenticator(object):
    '''
    Compatibility wrapper around AuthenticatorAPI to maintain USB/IP compatibility.
    '''

    @classmethod
    def get_pin_cose_key(cls, pin_req, cid):
        return AuthenticatorAPI.get_pin_cose_key(pin_req, cid)

    @classmethod
    def get_pin_retries(cls, pin_req, cid):
        return AuthenticatorAPI.get_pin_retries(pin_req, cid)

    @classmethod
    def get_pin_token(cls, pin_req, cid):
        return AuthenticatorAPI.get_pin_token(pin_req, cid)
    ...
    def attestation_out(cls, clientDataHash, rp, user, pkCredsParams, excludeList, exts, cid):
        error, authData, attStmt = AuthenticatorAPI.attestation_out(
            clientDataHash, rp, user, pkCredsParams, excludeList, exts, cid)
        if error:
            return None, None  # Return old format for compatibility
    ...
    def assertion_out(cls, rpId, clientDataHash, allowedList, exts, cid):
        error, credential, authData, signature, userHandle = AuthenticatorAPI.assertion_out(
            rpId, clientDataHash, allowedList, exts, cid)
        if error:
            return None, None, None, None  # Return old format for compatibility
        return credential, authData, signature, userHandle
...
class CBORCommand(object):
    ...
    def unpack(self):
        if self.cmd is None:
            return list((self.CBORStatusCode.CTAP1_ERR_INVALID_COMMAND).to_bytes())
        ...
        if handler:
            return handler(bytes(self.request))
        else:
            return list((self.CBORStatusCode.CTAP1_ERR_INVALID_COMMAND).to_bytes())
...
class CTAP2HIDevice(USBDevice):
    ...
    def __init__(self):
        ...
        # Initialize the new AuthenticatorAPI
        AuthenticatorAPI()
